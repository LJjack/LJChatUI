...
@implementation NSAttributedString (LJEmoji)
...
+ (NSAttributedString *)lj_formateContent:(NSString *)comment attributes:(NSDictionary *)attributes {
    
    NSString *regex = @"\\[([\u4E00-\u9FA5OKN]{1,3})\\]";
    NSRange range = [comment rangeOfString:regex options:NSRegularExpressionSearch];
    
    if (!range.length) {
        NSMutableAttributedString *contentAttributedString = [[NSMutableAttributedString alloc]initWithString:comment attributes:attributes];
    ...
    NSMutableAttributedString *contentAttributedString = [[NSMutableAttributedString alloc]initWithString:originString attributes:attributes];
