...
//每秒刷新30次
#define kReduceWidth (([UIScreen mainScreen].bounds.size.width)/kRecordMaxTime/kRefreshRate)

typedef void (^VonvertSucc)(NSString *path);

typedef void(^ChangeFocusSucc)(AVCaptureDevice *captureDevice);

@interface LJRecordVideoView()<AVCaptureFileOutputRecordingDelegate>

@property (nonatomic, strong) UIButton    *recordBtn;
@property (nonatomic, strong) UIButton    *cancelBtn;
@property (nonatomic, strong) UIView      *upSlidePanel;
@property (nonatomic, strong) UIImageView *upSlidePic;
@property (nonatomic, strong) UILabel     *upSlideText;
@property (nonatomic, strong) UIImageView *touchUpCancel;
...
@implementation LJRecordVideoView
...
-(void)singleTap:(UITapGestureRecognizer *)tapGesture {
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.05 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        [weakSelf layoutFoucsView:point];
    });
...
#pragma clang diagnostic pop
                        ...
                        [weakSelf onCancelBtn:_cancelBtn];
                        return;
                    }
                });
            }];
...
- (void)addPreviewLayer {
    ...
    NSLog(@"设备的方向 %@",@(_previewLayer.connection.videoOrientation));
...
- (void)captureOutput:(AVCaptureFileOutput *)captureOutput didFinishRecordingToOutputFileAtURL:(NSURL *)outputFileURL fromConnections:(NSArray *)connections error:(NSError *)error {
    if (!_isCancelRecord) {
        ...
        NSString *nsTmpDIr = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES)[0];
        NSString *videoPath = [nsTmpDIr stringByAppendingPathComponent:[NSString stringWithFormat:@"record_video_mp4_%1.f.mp4", [NSDate timeIntervalSinceReferenceDate]]];
        NSLog(@"视频压缩 前 大小 %@M", @([NSData dataWithContentsOfURL:outputFileURL].length / 1024. / 1024.));
        ...
        [self convertToMP4:urlAsset videoPath:videoPath succ:^(NSString *path) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                if ([weakSelf.delegate respondsToSelector:@selector(recordVideoViewTouchUpDone:)]) {
                    [weakSelf.delegate recordVideoViewTouchUpDone:path];
                }
                [self onCancelBtn:_cancelBtn];
            });
            ...
            switch ([exportSession status]) {
                ...
                default:
                    if (succ) {
                        ...
                        NSLog(@"视频压缩 后 大小 %@M", @(data.length / 1024. / 1024.));
                        succ(videoPath);
                    }
                    break;
            }
