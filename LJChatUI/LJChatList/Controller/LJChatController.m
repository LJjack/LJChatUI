...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    TIMConversation *chattingConversation = self.dataList[indexPath.row];
    [LJIMManagerListener sharedInstance].chattingConversation = chattingConversation;
    [[LJIMManagerListener sharedInstance] openNewConversation];
    
    [self performSegueWithIdentifier:@"showMessageC" sender:chattingConversation];
}
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    if ([segue.identifier isEqualToString:@"showMessageC"]) {
        
        LJMessageController *msgC = segue.destinationViewController;
        LJMessagesModel *model = [LJMessagesModel sharedInstance];
        model.chatingConversation = sender;
        msgC.msgModel = model;
    }
}
