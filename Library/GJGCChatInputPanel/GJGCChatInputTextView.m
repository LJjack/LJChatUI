...
@property (nonatomic,copy)GJGCChatInputTextViewDidBecomeFirstBJResponseBlock BJResponseBlock;
...
- (void)initSubViewsWithFrame:(CGRect)frame
{
    ...
    self.placeHolderLabel.textColor = [UIColor colorWithRed:144/255.0 green:144/255.0 blue:144/255.0 alpha:1.0];
    ...
    [self.recordButton setBackgroundImage:[self imageForColor:[UIColor colorWithWhite:0 alpha:0.1] withSize:self.recordButton.gjcf_size] forState:UIControlStateHighlighted];
...
- (void)observeRequiredNoti
{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(observeRecordSoundMeter:) name:soundMeterNoti object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(observeRecordTooShort:) name:recordTooShortNoti object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(observeRecordTooLong:) name:recordTooLongNoti object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(observeSetLastMessageDraft:) name:setMessageDraftNoti object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(observeEmojiPanelChooseDeleteNoti:) name:emojiDeleteNoti object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(observeEmojiPanelChooseEmojiNoti:) name:chooseEmojiNoti object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(observeAppendFocusOnOther:) name:appendTextNoti object:nil];
...
- (void)observeAppendFocusOnOther:(NSNotification *)noti
{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:formateNoti object:self.textView.text];
...
- (void)setInputTextBackgroundImage:(UIImage *)inputTextBackgroundImage
{
    ...
    self.inputBackgroundImageView.image = [_inputTextBackgroundImage resizableImageWithCapInsets:UIEdgeInsetsMake(2, 2, 2, 2)];
}
...
- (void)setRecordAudioBackgroundImage:(UIImage *)recordAudioBackgroundImage
{
    ...
    [self.recordButton setBackgroundImage:[_recordAudioBackgroundImage resizableImageWithCapInsets:UIEdgeInsetsMake(2, 2, 2, 2)] forState:UIControlStateNormal];
}
...
{    
    NSString *formateNoti = [GJGCChatInputConst panelNoti:GJGCChatInputTextViewContentChangeNoti formateWithIdentifier:self.panelIdentifier];
    [[NSNotificationCenter defaultCenter] postNotificationName:formateNoti object:textView.text];
...
- (BOOL)textView:(UITextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text
{
    ...
    if ([text isEqualToString:@"\n"]) {
        NSString *trimString = [self.textView.text stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]];
        /* 是否纯空格 */
        BOOL isAllWhiteSpace = !trimString.length;
        if (isAllWhiteSpace) {
...
{
    if (self.BJResponseBlock) {
        self.BJResponseBlock(self);
    }
...
- (void)configTextViewDidBecomeFirstResponse:(GJGCChatInputTextViewDidBecomeFirstBJResponseBlock)firstBJResponseBlock
{
    if (self.BJResponseBlock) {
        self.BJResponseBlock = nil;
    }
    self.BJResponseBlock = firstBJResponseBlock;
}
