...
- (instancetype)initWithPanelDelegate:(id<GJGCChatInputPanelDelegate>)aDelegate;
{
    if (self = [super init]) {
        ...
        _panelIndentifier = [NSString stringWithFormat:@"GJGCChatInputPanel_%@",[self currentTimeStampString]];
        
        self.emojiPanel = [[GJGCChatInputExpandEmojiPanel alloc]initWithFrame:CGRectMake(0, self.inputBarHeight, [UIScreen mainScreen].bounds.size.width, 216)];
...
- (instancetype)initForCommentBarWithPanelDelegate:(id<GJGCChatInputPanelDelegate>)aDelegate
{
    if (self = [super init]) {
        ...
        self.emojiPanel = [[GJGCChatInputExpandEmojiPanel alloc]initWithFrameForCommentBarStyle:CGRectMake(0, self.inputBarHeight, [UIScreen mainScreen].bounds.size.width, 216)];
...
- (void)initSubViews
{
    ...
    /* 输入条 */
    self.inputBar = [[GJGCChatInputBar alloc]initWithFrame:(CGRect){0,0,[UIScreen mainScreen].bounds.size.width,self.inputBarHeight}];
    ...
    /* 表情面板 */
    self.emojiPanel.backgroundColor = [UIColor colorWithRed:252/255.0 green:252/255.0 blue:252/255.0 alpha:1.0];
    self.emojiPanel.panelIdentifier = self.panelIndentifier;
    [self addSubview:self.emojiPanel];
    
    /* 扩展面板 */
    CGRect menuPanel = self.emojiPanel.frame;
    self.menuPanel = [[GJGCChatInputExpandMenuPanel alloc] initWithFrame:menuPanel withDelegate:self];
    self.menuPanel.backgroundColor = [UIColor colorWithRed:252/255.0 green:252/255.0 blue:252/255.0 alpha:1.0];
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(updateInputTextContent:) name:formateNoti object:nil];
...
- (void)observePanelInnerEventNoti
{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(observeEmojiPanelSend:) name:formateNoti object:nil];
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(observeGifEmojiPanelSend:) name:formateGifSendNoti object:nil];
...
{
    NSString *trimString = [self.messageDraft stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]];
    /* 是否纯空格 */
...
- (void)setLastMessageDraft:(NSString *)msgDraft
{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:formateNoti object:msgDraft];
}
...
- (void)appendFocusOnOther:(NSString *)otherName
{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:formateNoti object:otherName];
